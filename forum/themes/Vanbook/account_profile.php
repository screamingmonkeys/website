	...
	echo '<ul class="vcard">';
		...
		if ($this->User->DisplayIcon != '') {
			echo '<li class="ProfileTitle WithIcon clearfix">
				<ul class="ProfileHeader">
					<li class="ProfileIcon" style="background-image:url(\''.$this->User->DisplayIcon.'\')">&nbsp;</li>';
		} else {
			echo '<li class="ProfileTitle clearfix">
					<ul class="ProfileHeader">';
		}
			echo '<li class="ProfileName">'.$this->User->Name.'</li>
					<div class="ProfileRole">'.$this->User->Role.'</div>
				</ul>
		</li>';
		...
		echo '<li class="Picture" style="background-image: url(themes/Vanbook/styles/default/default-pict.png)"></li>';
		echo '<li class="ProfileInfo">
			'.$this->Context->GetDefinition('Information').'
			</li>';
