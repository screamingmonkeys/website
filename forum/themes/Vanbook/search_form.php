...
echo '<div id="Form" class="Search">
	...
	echo '<fieldset id="SearchDiscussionFields">
		...
		$this->Render_PostBackForm('SearchDiscussions', 'get');
			echo '<ul>
				<li><label for="txtDiscussionKeywords">'.$this->Context->GetDefinition('FindDiscussionsContaining').'</label></li>
				<li><input id="txtDiscussionKeywords" type="text" name="Keywords" value="'.($this->Search->Type == 'Topics'?$this->Search->Query:'').'" class="SearchInput AdvancedSearchInput" /></li>
				';
	...
	echo '<fieldset id="SearchCommentFields">
		...
		$this->Render_PostBackForm('SearchComments', 'get');
			echo '<ul>
				<li><label for="txtCommentKeywords">'.$this->Context->GetDefinition('FindCommentsContaining').'</label></li>
				<li><input id="txtCommentKeywords" type="text" name="Keywords" value="'.($this->Search->Type == 'Comments'?$this->Search->Query:'').'" class="SearchInput AdvancedSearchInput" /></li>
				';
				if ($this->Context->Configuration['USE_CATEGORIES']) {
	...
	echo '<fieldset id="SearchUserFields">
		...
		$this->Render_PostBackForm('SearchUsers', 'get');
			echo '<ul>
				<li><label for="txtUserKeywords">'.$this->Context->GetDefinition('FindUserAccountsContaining').'</label></li>
				<li><input id="txtUserKeywords" type="text" name="Keywords" value="'.($this->Search->Type == 'Users'?$this->Search->Query:'').'" class="SearchInput AdvancedSearchInput" /></li>
				<li>
