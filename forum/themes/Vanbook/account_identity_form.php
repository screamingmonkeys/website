			...
			<ul id="CustomInfo" class="clearfix">';
				...
				if (count($this->User->Attributes) > 0) {
					...
					for ($i = 0; $i < $AttributeCount; $i++) {
						if ($i == 0) {
							echo '<li class="LabelHeader">'.$this->Context->GetDefinition('Label').'</li>
							<li class="ValueHeader">'.$this->Context->GetDefinition('Value').'</li>';
						}
						echo '<li class="Label"><input type="text" name="Label'.$CurrentItem.'" value="'.$this->User->Attributes[$i]['Label'].'" maxlength="20" class="LVLabelInput" /></li>
						<li class="Value"><input type="text" name="Value'.$CurrentItem.'" value="'.$this->User->Attributes[$i]['Value'].'" maxlength="200" class="LVValueInput" /></li>';
						$CurrentItem++;
					}
				} else {
					echo '<li class="LabelHeader">'.$this->Context->GetDefinition('Label').'</li>
