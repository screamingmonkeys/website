...
var mooSlidingTabs = new Class({
	Implements: [Options],
	
	//set class options
	options: {		
		buttonContainer:	 'navigation',	// contains buttons
		slideContainer:		 'slider',		// contains slides							 
    	...
    	slideEffectDuration: 400, 			// options for effect used to animate the sliding, 0.4 of a second
	...
	initialize: function(options){  
		this.setOptions(options);	
  		...
  		if (this.options.buttonContainer) { this.buttons = $(this.options.buttonContainer).getChildren(); }
    	this.outerSlidesBox = $(this.options.slideContainer);
    	...
    	this.heightFx = new Fx.Tween(this.outerSlidesBox).set('height', this.options.slideEffect);
...
window.addEvent('domready', function(){
	var Sliders = new mooSlidingTabs({
		buttonContainer: 'navigation',
		slideContainer: 'panes_content'
	});
});